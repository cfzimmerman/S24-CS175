...
// and through a runtime_error exception. 8857
void checkGlErrors();
...
// Read and compile a pair of vertex shader and fragment shader source codes
// from memory into a GL shader program. Throws runtime_error on error
...
void readAndCompileSingleShader(GLuint shaderHandle,
                                const char *shaderFileName);

// Reads and compiles a single shader (vertex, fragment, etc) from memory into a
// GL shader. Throws runtime_error on error
...
class GlTexture : Noncopyable {
  ...
  public:
    ...
    ~GlTexture() { glDeleteTextures(1, &handle_); }
...
class GlBufferObject : Noncopyable {
  ...
  public:
    ...
    ~GlBufferObject() { glDeleteBuffers(1, &handle_); }

    // Casts to GLuint so can be used directly glBindBuffer and so on
    operator GLuint() const { return handle_; }
};

// Light wrapper around a GL vertex array object handle that automatically
// allocates and deallocates. Can be casted to a GLuint.
class GlArrayObject : Noncopyable {
  protected:
    GLuint handle_;
  ...
  public:
    ...
    ~GlArrayObject() { glDeleteVertexArrays(1, &handle_); }
...
inline GLint safe_glGetUniformLocation(const GLuint program,
                                       const char varname[]) {
    GLint r = glGetUniformLocation(program, varname);
    if (r < 0)
        std::cerr << "WARN: " << varname
                  << " cannot be bound (it either doesn't exist or has been "
                     "optimized away). safe_glUniform calls will silently "
                     "ignore it.\n"
                  << std::endl;
    return r;
}

inline GLint safe_glGetAttribLocation(const GLuint program,
                                      const char varname[]) {
    ...
    if (r < 0)
        std::cerr
            << "WARN: " << varname
            << " cannot be bound (it either doesn't exist or has been "
               "optimized away). safe_glAttrib calls will silently ignore it.\n"
            << std::endl;
    return r;
}
...
inline void safe_glUniform3i(const GLint handle, const GLint a, const GLint b,
                             const GLint c) {
    if (handle >= 0)
        glUniform3i(handle, a, b, c);
}

inline void safe_glUniform4i(const GLint handle, const GLint a, const GLint b,
                             const GLint c, const GLint d) {
    if (handle >= 0)
...
inline void safe_glUniform2f(const GLint handle, const GLfloat a,
                             const GLfloat b) {
    if (handle >= 0)
        glUniform2f(handle, a, b);
}

inline void safe_glUniform3f(const GLint handle, const GLfloat a,
                             const GLfloat b, const GLfloat c) {
    if (handle >= 0)
        glUniform3f(handle, a, b, c);
}

inline void safe_glUniform4f(const GLint handle, const GLfloat a,
                             const GLfloat b, const GLfloat c,
                             const GLfloat d) {
    if (handle >= 0)
...
inline void safe_glVertexAttribPointer(const GLint handle, GLint size,
...
inline void safe_glVertexAttrib2f(const GLint handle, const GLfloat a,
                                  const GLfloat b) {
    if (handle >= 0)
        glVertexAttrib2f(handle, a, b);
}

inline void safe_glVertexAttrib3f(const GLint handle, const GLfloat a,
                                  const GLfloat b, const GLfloat c) {
    if (handle >= 0)
        glVertexAttrib3f(handle, a, b, c);
}

inline void safe_glVertexAttrib4f(const GLint handle, const GLfloat a,
                                  const GLfloat b, const GLfloat c,
                                  const GLfloat d) {
    if (handle >= 0)
...
inline void safe_glVertexAttrib4Nub(const GLint handle, const GLubyte a,
                                    const GLubyte b, const GLubyte c,
                                    const GLubyte d) {
    if (handle >= 0)
