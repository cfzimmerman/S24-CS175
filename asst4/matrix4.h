...
inline Matrix4 normalMatrix(const Matrix4 &m) { // get the normal matrix (v332)
  Matrix4 invm = inv(m);
  invm(0, 3) = invm(1, 3) = invm(2, 3) = 0;
  return transpose(invm);
}

/// Returns a Matrix4 representing only the translational
/// component of the given matrix.
...
/// Returns a Matrix4 representing only the rotational component
/// of the given matrix.
inline Matrix4 linFact(const Matrix4 &m) {
  Matrix4 mtx = Matrix4();
  for (size_t row = 0; row < 3; row++) {
    for (size_t col = 0; col < 3; col++) {
      mtx(row, col) = m(row, col);
    }
  }
  return mtx;
}
