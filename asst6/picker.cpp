...
Picker::Picker(const RigTForm &initialRbt, const ShaderState &curSS)
    : drawer_(initialRbt, curSS), idCounter_(0), srgbFrameBuffer_(true) {}
...
bool Picker::visit(SgShapeNode &node) {
  ...
  // it's invariant that Shape nodes are only parented by Transform nodes
  assert(parent != NULL);
  addToMap(idCounter_, parent);

  Cvec3 colorId = idToColor(idCounter_);
  safe_glUniform3f(drawer_.getCurSS().h_uIdColor, colorId(0), colorId(1),
                   colorId(2));
...
shared_ptr<SgRbtNode> Picker::getRbtNodeAtXY(int x, int y) {
  ...
  PackedPixel rgb = {pixels[0], pixels[1], pixels[2]};
  return find(colorToId(rgb));
}
...
Cvec3 Picker::idToColor(int id) {
  ...
  if (!srgbFrameBuffer_)
    ...
    Cvec3 linearColor;
    for (int i = 0; i < 3; ++i) {
      linearColor[i] = framebufferColor[i] <= 0.04045
