...
public:
  ...
  std::shared_ptr<SgNode> getChild(int i) { return children_[i]; }
...
RigTForm getPathAccumRbt(std::shared_ptr<SgTransformNode> source,
...
template <typename Geometry> class SgGeometryShapeNode : public SgShapeNode {
  std::shared_ptr<Geometry> geometry_;
  Matrix4 affineMatrix_;
  Cvec3 color_;

public:
  SgGeometryShapeNode(std::shared_ptr<Geometry> geometry, const Cvec3 &color,
                      const Cvec3 &translation = Cvec3(0, 0, 0),
                      const Cvec3 &eulerAngles = Cvec3(0, 0, 0),
                      const Cvec3 &scales = Cvec3(1, 1, 1))
      : geometry_(geometry), color_(color),
  ...
  virtual Matrix4 getAffineMatrix() { return affineMatrix_; }
